...
class Anonymiser:
    ...
    def anonymise(schema_model, data, method, manual, fields):
        ...
        if manual == True:
            ...
            name_match_pairs = synth.match_fields(schema_model, method)[0]
            field_names = [name for name in name_match_pairs.keys() if name != ""]
        new_schema_model = Anonymiser.subset_model(schema_model, field_names)

        return_data = dict(synth.synthesise(new_schema_model, method)[0])
