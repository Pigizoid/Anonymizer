...
def infer_json_args(property: Dict[str, Any]):
    ...
    if "items" in property:
    ...
    elif infer_json_type(property) is dict:
        ...
        if "additionalProperties" in property:
            ...
            data_args[0] = {
                "pattern": list(pattern_properties.keys())[0],
                "type": "string",
            }
    ...
    if isinstance(schema_model, list):
        ...
        for key, value in schema_model.items():
            ...
            if key == "$anchor" or key == "$defs" or key == "$id":
                definitions = walk_json_schema_defs(
                    value, definitions=definitions, path=recursed_path
                )
                if key == "$defs":
...
def walk_json_schema_refs(schema_model, references=None, path="", name=""):
    ...
    if isinstance(schema_model, list):
        for i, item in enumerate(schema_model):
            recursed_path = path + f"({i})"
            references = walk_json_schema_refs(
                item, references=references, path=recursed_path
            )
    elif isinstance(schema_model, dict):
        for key, value in schema_model.items():
            if key == "$ref":
                ...
                recursed_path = path + f"[{key}]"
                references = walk_json_schema_refs(
                    value, references=references, path=recursed_path
                )
    return references
...
def ref_pass_model(schema_model: JsonSchemaClass):
    definitions = walk_json_schema_defs(
        schema_model.contents, name=schema_model.__name__
    )
    references = walk_json_schema_refs(
        schema_model.contents, name=schema_model.__name__
    )
...
def ref_pass_openapi_model(schema_model: Dict[str, Any], name: str):
    definitions = [
        ...
        for path in walk_json_schema_defs(schema_model, name=name)
    ]
    references = [
        "[components][schemas]" + path
        for path in walk_json_schema_refs(schema_model, name=name)
    ]
    return (references, definitions)
...
def load_parse_json_schema(schema_model: JsonSchemaClass):
    ...
    for built_def in built_defs.values():
        if "$defs" in built_def:
            built_def.pop("$defs")
    needed_built_defs = {
        name: strip_json_schema_ids(content)
        for name, content in built_defs.items()
        if name in return_json_schema_ref_names(new_schema_model)
    }
...
def load_schemas_from_openapi(schema_model: Dict[str, Any]):
    ...
    schemas = [
        ref_pass_openapi_model(schema, name)
        for name, schema in new_schema_model["components"]["schemas"].items()
    ]
    schemas = [
        [reference for schema0 in schemas for reference in schema0[0]],
        [definition for schema1 in schemas for definition in schema1[1]],
    ]
    ...
    for name, schema in new_schema_model["components"]["schemas"].items():
        needed_built_defs = {
            name: strip_json_schema_ids(content)
            for name, content in built_defs.items()
            if name in return_json_schema_ref_names(schema)
        }
    ...
    return_schemas = {
        name: strip_json_schema_ids(content)
        for name, content in new_schema_model["components"]["schemas"].items()
    }
...
def validate_instance_data(
    instance: Dict, schema: List[Dict], schema_instances=None
) -> None:
