...
def test_anonymise_data_perturb():
    return_data = anonymise_data(0,input_data,anon_methods)
    assert type(return_data['field_0']) == type(input_data['field_0'])
    assert type(return_data['field_1']) == type(input_data['field_1'])
    assert type(return_data['field_2']) == type(input_data['field_2'])
    assert type(return_data['field_3']) == type(input_data['field_3'])
    assert type(return_data['field_4']) == type(input_data['field_4'])
    assert type(return_data['field_5']) == type(input_data['field_5'])
    assert all([type(return_data['list'][x]) == type(input_data['list'][x]) for x in range(len(input_data['list']))])
    assert all([type(x1) == type(x2) for x1,x2 in zip(return_data['tuple'],input_data['tuple'])])
    assert set([type(x) for x in return_data['set']]) == set([type(x) for x in input_data['set']])
    assert all([type(return_data['dict'][f"field_{x}"]) == type(input_data['dict'][f"field_{x}"]) for x in range(len(input_data['dict']))])
